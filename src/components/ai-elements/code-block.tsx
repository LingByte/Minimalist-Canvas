...
function ShikiCodeView(props: {
  code: string
  language: BundledLanguage | string
}) {
  const [html, setHtml] = useState<string | null>(null)
  const language = getRequestedCodeLanguage(props.language)
  ...
  if (!html) {
    return (
      <pre className='m-0 p-4 font-mono text-[13px] leading-6'>
  ...
  return (
    <div
      className={cn(
        'shiki-host max-w-full overflow-auto',
        '[&_pre.shiki]:m-0 [&_pre.shiki]:!bg-transparent [&_pre.shiki]:p-4',
        '[&_code]:font-mono [&_code]:text-[13px] [&_code]:leading-6 [&_code]:whitespace-pre'
      )}
  ...
  return (
    <CodeBlockContext.Provider value={{ code, language: displayLanguage }}>
      <CodeBlockFrame
        ...
        {...props}
      >
        <ShikiCodeView code={code} language={language} />
      </CodeBlockFrame>
