...
export function SliderCaptcha({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only re-apply when session id changes
  }, [session?.id, autoLoad])
    ...
    (nextOffset: number) => {
      ...
      if (ratio >= PASS_RATIO) {
        ...
        const scaled = Math.min(
          trackWidth,
          Math.max(0, Math.round(ratio * trackWidth))
        )
        onChangeRef.current({
          captchaId,
          captchaType: 'slider',
    ...
    <div className='captcha-slider-root select-none'>
      ...
      >
        ...
        <span
          ...
          style={{
            opacity: passed ? 0 : Math.max(0.25, 1 - progress / 100),
          }}
        >
          {loading
            ? t('Loading')
            : passed
              ? t('Verified')
              : t('Hold and drag the slider to the right')}
        </span>
        <button
          type='button'
          aria-label={t('Hold and drag the slider to the right')}
      ...
      {!session ? (
        <p className='mt-2 text-xs text-neutral-400'>
          {passed
            ? t('Verified')
            : t('Drag the slider to complete verification')}
        </p>
      ) : null}
    </div>
  )
