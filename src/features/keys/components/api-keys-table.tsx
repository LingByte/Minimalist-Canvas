...
// react-router adapter for useTableUrlState (TanStack-style search/navigate)
function parseSearchParam(value: string): unknown {
  try {
...
function useReactRouterTableUrl() {
  const [searchParams, setSearchParams] = useSearchParams()
  const search = useMemo(() => {
    const record: Record<string, unknown> = {}
    for (const [key, value] of searchParams.entries()) {
      record[key] = parseSearchParam(value)
    }
    return record
  }, [searchParams])
    ...
    setSearchParams(
      (prev) => {
        ...
        for (const [key, value] of prev.entries()) {
          prevRecord[key] = parseSearchParam(value)
        }
        const patch =
      ...
      {API_KEYS_MOBILE_SKELETON_IDS.map((id) => (
        ...
        >
          ...
          <div className='flex items-center justify-between gap-3'>
            <Skeleton.Input active size='small' style={{ width: 176 }} />
            <Skeleton.Button active size='small' style={{ width: 64 }} />
          </div>
  ...
  if (!rows.length) {
    return (
      <div className='rounded-lg border p-8'>
        <Empty
          image={<DatabaseOutlined style={{ fontSize: 24 }} />}
          description={
            <div>
              <div className='font-semibold'>{t('No API Keys Found')}</div>
...
export function ApiKeysTable() {
  ...
  const { search, navigate } = useReactRouterTableUrl()
