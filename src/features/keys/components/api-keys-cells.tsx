...
export function ApiKeyCell({ apiKey }: { apiKey: ApiKey }) {
  ...
  if (isLoading) {
    copyIcon = <LoadingOutlined spin />
    copyTooltip = t('Loading...')
  } else if (isCopied) {
    copyIcon = <CheckOutlined className='text-green-600' />
    ...
    <div className='flex max-w-full min-w-0 items-center'>
      ...
      >
        <Button
          type='text'
          size='small'
          className='text-muted-foreground h-7 max-w-full min-w-0 justify-start truncate px-0 font-mono text-xs'
        >
...
export function ModelLimitsCell({ apiKey }: { apiKey: ApiKey }) {
  ...
  return (
    <Tooltip
      placement='top'
      title={
        <div className='max-h-[200px] max-w-xs space-y-0.5 overflow-y-auto text-xs'>
          {models.map((m) => (
