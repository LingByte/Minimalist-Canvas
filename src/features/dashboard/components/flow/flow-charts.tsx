  ...
  if (displayState === 'loading') {
    ...
    chartContent = (
      <div className='flex h-full items-center justify-center py-12'>
        <Empty
          image={<Route className='text-muted-foreground size-10' />}
          description={
            <div className='space-y-1'>
              <div className='text-foreground font-medium'>
                {t('No flow data available')}
              </div>
        ...
        <div className='flex min-w-0 flex-wrap items-end gap-2'>
          <div className='flex min-w-0 flex-col gap-1.5'>
            <div className='flex items-center gap-1.5'>
              ...
              <Tooltip title={t('Choose how flow widths are calculated.')}>
                <button
                  type='button'
            ...
            <Segmented
              ...
              value={metric}
              aria-label={t('Flow width metric')}
              onChange={(value) => setMetric(value as FlowMetric)}
              options={FLOW_METRIC_OPTIONS.map((option) => {
          ...
          <div className='flex min-w-0 flex-col gap-1.5'>
            ...
            <Segmented
              ...
              aria-label={t('Display limit')}
              onChange={(value) => setTopNodeLimit(Number(value))}
              options={FLOW_TOP_LIMIT_OPTIONS.map((limit) => ({
                value: String(limit),
          ...
          <div className='flex min-w-0 flex-col gap-1.5'>
            ...
            <Segmented
              size='small'
              value={overflowMode}
              aria-label={t('Overflow items')}
              onChange={(value) =>
                setOverflowMode(value as FlowOverflowMode)
              }
              options={FLOW_OVERFLOW_MODE_OPTIONS.map((option) => ({
                value: option.value,
      ...
      <div className='overflow-hidden rounded-lg border'>
        <div className='flex w-full flex-col gap-2 border-b px-3 py-2 sm:px-5 sm:py-3 lg:flex-row lg:items-center lg:justify-between'>
          ...
          <div className='flex min-w-0 items-center gap-1 overflow-x-auto pb-1 lg:justify-end lg:pb-0'>
              <Tooltip title={t('Click a stage to show or hide that column')}>
                <button
                  type='button'
                  ...
                  <Fragment key={stage}>
                    ...
                    <Tooltip title={t(meta.descKey)}>
                      <Button
                        ...
                        icon={!visible ? <EyeOff className='size-3' /> : undefined}
                      >
                        {t(meta.labelKey)}
                      </Button>
                    </Tooltip>
