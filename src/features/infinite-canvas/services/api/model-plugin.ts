...
import { VIDEO_POLL_INTERVAL_MS, VIDEO_POLL_TIMEOUT_MS } from "@canvas/constant/video-generation";
import i18n from "@canvas/i18n";
import { buildApiUrl, type AiConfig, type ModelCapability } from "@canvas/stores/use-config-store";
...
function createPoll(signal?: AbortSignal) {
    return async function poll<T, R>(request: () => Promise<T>, extract: (value: T) => R | null | undefined | false, options?: PluginPollOptions): Promise<R> {
        ...
        const timeoutMs = options?.timeoutMs ?? VIDEO_POLL_TIMEOUT_MS;
...
export function getPluginVariables(): PluginVariable[] {
    return [
        ...
        { name: "videos", type: "string[]", desc: i18n.t("modelPlugin.variables.videos"), capabilities: ["video"] },
...
const task = await request({
  ...
  data: {
    ...
    ...(images.length === 1 ? { image: images[0] } : images.length > 1 ? { images } : {}),
...
return await poll(
  ...
  (state) => state.status === "completed" ? { url: state.result_url || state.url || state.video_url } : null,
