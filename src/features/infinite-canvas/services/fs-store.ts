...
function workspaceSegment(projectId: string) {
    return sanitizeSegment(projectId) || "default";
}

async function workspacePath(projectId: string) {
    return joinPath(await storageRoot(), "workspaces", workspaceSegment(projectId));
}
...
export async function canvasWorkspaceDir(projectId: string): Promise<string> {
    const dir = await workspacePath(projectId);
    if (!removedWorkspaces.has(workspaceSegment(projectId))) await ensureDir(dir);
    return dir;
}
...
export async function writeCanvasWorkspace(projectId: string, title: string, snapshot: unknown): Promise<void> {
    if (!isTauri()) return;
    const key = workspaceSegment(projectId);
    await enqueue(`workspace:${key}`, async () => {
        if (removedWorkspaces.has(key)) return;
        const dir = await workspacePath(projectId);
        await ensureDir(dir);
        if (removedWorkspaces.has(key)) return;
...
export async function syncWorkspaceMedia(
    ...
    await enqueue(`workspace:${key}`, async () => {
        if (removedWorkspaces.has(key)) return;
        const mediaDir = await joinPath(await workspacePath(projectId), "media");
...
/** Delete the on-disk workspace created for a canvas. Queued behind any in-flight mirror. */
export async function removeCanvasWorkspace(projectId: string): Promise<void> {
    if (!isTauri() || !projectId) return;
    const key = workspaceSegment(projectId);
    removedWorkspaces.add(key);
    await enqueue(`workspace:${key}`, async () => {
        await removePath(await workspacePath(projectId));
    });
}
