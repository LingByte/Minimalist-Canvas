...
import { generateCanvasImage, waitForGenerationAsset } from "@canvas/services/api/canvas-image-job";
...
import {
    ...
    syncCanvasImageGenerationAsset,
    syncCanvasVideoGenerationAsset,
} from "@canvas/services/generation-asset-sync";
...
import { buildNodeMentionReferences, type CanvasResourceReference } from "@canvas/lib/canvas/canvas-resource-references";
...
import { findContainingGroupId, findGroupDropTarget, getConnectionTargetAnchor, normalizeConnection, snapNodesIntoGroup } from "@canvas/lib/canvas/canvas-node-geometry";
import {
    audioExtension,
    buildAngleLabel,
    buildAnglePrompt,
    ...
    isRecoverableImageGeneration,
    isRecoverableVideoGeneration,
...
import type { ReferenceAudio } from "@canvas/types/media";
...
function videoWaitOptions(
    signal: AbortSignal | undefined,
    onProgress: (progress: number) => void,
    ignoreResultUrls?: string[],
): { signal?: AbortSignal; onProgress: (progress: number) => void; ignoreResultUrls?: string[] } {
    return {
        signal,
        onProgress,
...
function mergeReferenceUrlLists(...groups: Array<string[] | undefined>) {
    const urls: string[] = [];
...
function referenceImageHttpUrls(refs?: Array<{ url?: string; storageKey?: string; dataUrl?: string } | null | undefined>) {
    return mediaRefHttpUrls(refs);
}

function mediaRefHttpUrls(refs?: Array<{ url?: string; storageKey?: string; dataUrl?: string } | null | undefined>) {
    if (!refs?.length) return [];
    return mergeReferenceUrlLists(
        refs.flatMap((item) => {
            if (!item) return [];
            return [item.url, item.storageKey, item.dataUrl].filter((value): value is string => Boolean(value));
        }),
    );
}

function canvasVideoAssetConfigFromAi(
    generationConfig: {
        size?: string;
        videoSeconds?: string;
        vquality?: string;
    ...
    return buildCanvasVideoAssetConfig({
...
function canvasImageAssetConfigFromAi(
    generationConfig: { size?: string; quality?: string; background?: string; count?: string },
    opts?: { mode?: string; images?: string[]; count?: string | number },
) {
    return buildCanvasImageAssetConfig({
        size: generationConfig.size,
        quality: generationConfig.quality,
        background: generationConfig.background,
        mode: opts?.mode,
        count: opts?.count ?? generationConfig.count,
        images: opts?.images,
    });
}
...
function InfiniteCanvasPage() {
        ...
        return onCloudMediaUrl((storageKey, url) => {
            setNodes((prev) => {
                ...
                const next = prev.map((node) => {
                    ...
                    if (metadata.storageKey === storageKey && metadata.content !== url) {
                        nextMetadata = { ...nextMetadata, content: url };
                        metadataChanged = true;
                    }
                    if (metadata.images?.some((image) => image.storageKey === storageKey && image.content !== url)) {
                        nextMetadata = {
                            ...nextMetadata,
                            images: metadata.images.map((image) => (image.storageKey === storageKey ? { ...image, content: url } : image)),
                        };
                        metadataChanged = true;
                    }
    ...
    const resumedImageJobsRef = useRef(new Set<string>());
    ...
    const applyRemirroredCanvasVideo = useCallback(
        (
            nodeId: string,
            sync: {
                prompt?: string;
            ...
            (file: { url: string; storageKey: string; bytes: number; mimeType: string; width?: number; height?: number; durationMs?: number }) => {
                setNodes((prev) =>
                    prev.map((item) => (item.id === nodeId ? { ...item, metadata: { ...item.metadata, ...videoMetadata(file) } } : item)),
                );
    ...
    useEffect(() => {
        ...
        resumedVideoTasksRef.current.clear();
        resumedImageJobsRef.current.clear();
    ...
    const duplicateNode = useCallback((nodeId: string) => {
        ...
        // Reattach the same upstream inputs to the copy (incoming edges only).
    ...
    const copySelectedNodes = useCallback(() => {
        ...
        // Keep edges that feed into the selection (including from nodes outside the selection).
    ...
    const pasteCopiedNodes = useCallback(() => {
        ...
        const nextConnections = clipboard.connections.flatMap((connection) => {
            ...
            if (!toNodeId) return [];
            // Upstream may be outside the paste set — keep pointing at the original source node.
    ...
    const finishNodeDrag = useCallback((clientX?: number, clientY?: number) => {
        ...
        const wasClick = !didMove && dragRef.current.initialSelectedNodes.length === 1;
        ...
        if (didMove && clientX != null && clientY != null) {
        ...
        // Dragging over <video>/<audio controls> still synthesizes a click on mouseup; swallow it so play/pause is not toggled.
        if (didMove) {
            ...
            window.setTimeout(() => window.removeEventListener("click", swallowClick, true), 100);
        }
    ...
    const createVideoFileNode = useCallback(async (file: File, position: Position) => {
        const video = await uploadMediaFile(file, "video", { background: true });
    ...
    const createAudioFileNode = useCallback(async (file: File, position: Position) => {
        const audio = await uploadMediaFile(file, "audio", { background: true });
            ...
            if (node.type === CanvasNodeType.Video) {
                ...
                addAsset({
                    ...
                    data: {
                        ...
                        width: Math.round(Number(node.metadata?.naturalWidth) || node.width || 0),
                        height: Math.round(Number(node.metadata?.naturalHeight) || node.height || 0),
                    ...
                    metadata: {
                        ...
                        nodeId: node.id,
                        prompt: node.metadata?.prompt,
                        ...( /^https?:\/\//i.test(content) ? { upstreamUrl: content } : {}),
                    },
                });
                message.success(t("common.addedToAssets"));
                return;
        ...
        async (event: ReactChangeEvent<HTMLInputElement>) => {
            ...
            if (target?.nodeId) {
                ...
                if (isAudioFile(first)) {
                    const audio = await uploadMediaFile(first, "audio", { background: true });
                    const spec = NODE_DEFAULT_SIZE[CanvasNodeType.Audio];
                    setNodes((prev) =>
                        prev.map((node) =>
                    ...
                    const video = await uploadMediaFile(first, "video", { background: true });
                    ...
                    const image = await uploadImage(first, { background: true });
            ...
            message.success(t("canvas.projectPage.uploadedFiles", { count: files.length }));
            } catch (error) {
                console.error(error);
                message.error(error instanceof Error && error.message ? error.message : t("canvas.projectPage.uploadFailed"));
            } finally {
    ...
    const handleDrop = useCallback(
        async (event: ReactDragEvent<HTMLDivElement>) => {
            ...
            try {
                ...
                if (added) message.success(t("canvas.projectPage.uploadedFiles", { count: added }));
            } catch (error) {
                console.error(error);
                message.error(error instanceof Error && error.message ? error.message : t("canvas.projectPage.uploadFailed"));
            }
        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            if (sourceNode && builtinPanel?.writeBackToSelf && builtinPanel.mode === "image") {
                ...
                try {
                    ...
                    syncCanvasImageGenerationAsset({
                        ...
                        config: canvasImageAssetConfigFromAi(generationConfig, {
                            mode: refs.length ? "edit" : "generation",
                            images: referenceImageHttpUrls(refs),
                            count: 1,
                        }),
                    });
                    setNodes((prev) =>
            ...
            const generationContext = await hydrateNodeGenerationContext(
                buildNodeGenerationContext(
                    ...
                    // Video: keep the typed prompt as-is when the user already wrote one.
                    // Connected text nodes used to be appended blindly and polluted the scene text.
                    { appendUpstreamText: mode !== "video" || !prompt.trim() },
                ),
            );
            ...
            if (
                ...
                !generationContext.referenceVideos.length &&
                !generationContext.referenceAudios.length
            ) {
                finishGenerationRequest(nodeId, runController);
                setRunningNodeId(null);
            ...
            try {
                if (mode === "image") {
                    ...
                    let firstError = "";
                    let usedAnyServerJob = false;
                    await Promise.all(
                        imageIds.map(async (imageId) => {
                            try {
                                const { uploaded, usedServerJob } = await generateCanvasImage({
                                    clientId: imageId,
                                    ...
                                    signal: controller.signal,
                                    onServerJobStarted: () => {
                                        usedAnyServerJob = true;
                                        setNodes((prev) =>
                                            prev.map((node) =>
                                                node.id === rootId
                                                    ? { ...node, metadata: { ...node.metadata, imageJobRecoverable: true } }
                                                    : node,
                                            ),
                                        );
                                    },
                                });
                                if (usedServerJob) usedAnyServerJob = true;
                                ...
                                syncCanvasImageGenerationAsset({
                                    ...
                                    config: canvasImageAssetConfigFromAi(generationConfig, {
                                        mode: generationType,
                                        images: referenceImageHttpUrls(referenceImages),
                                        count: 1,
                                    }),
                                });
                                setNodes((prev) =>
                    ...
                    setNodes((prev) =>
                        prev.map((node) =>
                            node.id === rootId ? { ...node, metadata: { ...node.metadata, imageJobRecoverable: usedAnyServerJob } } : node,
                        ),
                    );
                ...
                if (mode === "video") {
                    ...
                    try {
                        ...
                        const ignoreResultUrls = generationReferenceUrls(generationContext);
                        const videoAssetConfig = canvasVideoAssetConfigFromAi(generationConfig, {
                            images: mediaRefHttpUrls(generationContext.referenceImages),
                            videos: mediaRefHttpUrls(generationContext.referenceVideos),
                            audios: mediaRefHttpUrls(generationContext.referenceAudios),
                        });
                        setNodes((prev) =>
                            prev.map((node) =>
                                node.id === videoId
                                    ? {
                                          ...
                                          metadata: {
                                              ...
                                              references: mergeReferenceUrlLists(node.metadata?.references, task.submittedReferenceUrls, ignoreResultUrls),
                                          },
                                      }
                                    : node,
                            ),
                        ...
                        const video = await storeGeneratedVideo(
                            await waitForVideoGenerationTask(
                                generationConfig,
                                task,
                                videoWaitOptions(controller.signal, (progress) => {
                                    setNodes((prev) =>
                                        prev.map((node) =>
                                            node.id === videoId
                                                ? { ...node, metadata: { ...node.metadata, generationProgress: progress } }
                                                : node,
                                        ),
                                    );
                                }, ignoreResultUrls),
                            ...
                            {
                                onRemirrored: applyRemirroredCanvasVideo(videoId, {
                                    prompt: effectivePrompt,
                                    model: generationConfig.model,
                                    taskId: task.id,
                                    attemptId: task.attemptId,
                        ...
                        await syncCanvasVideoGenerationAsset({
                            clientId: videoId,
                            prompt: effectivePrompt,
                            model: generationConfig.model,
                            taskId: task.id,
                                ...
                                node.id === videoId
                                    ? {
                                          ...
                                          metadata: {
                                              ...
                                              generateAudio: generationConfig.videoGenerateAudio,
                                              watermark: generationConfig.videoWatermark,
                                              references: mergeReferenceUrlLists(ignoreResultUrls, task.submittedReferenceUrls),
                                              videoTaskId: task.id,
                                              videoTaskProvider: task.provider,
                                              videoTaskModel: task.model,
                                              errorDetails: undefined,
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData, imageId?: string) => {
            ...
            const retryPromptSeed =
                sourceNode.metadata?.composerContent?.trim() ||
                sourceNode.metadata?.prompt?.trim() ||
                node.metadata?.prompt?.trim() ||
                "";
            const context = hasSavedImageMetadata
                ? null
                : await hydrateNodeGenerationContext(
                      buildNodeGenerationContext(sourceNode.id, nodesRef.current, connectionsRef.current, retryPromptSeed, {
                          appendUpstreamText: node.type !== CanvasNodeType.Video || !retryPromptSeed,
                      }),
                  );
            const prompt = (savedImageMetadata?.prompt || context?.prompt || node.metadata?.prompt || "").trim();
            if (!prompt && node.type !== CanvasNodeType.Video) {
                message.warning(t("canvas.projectPage.retryPromptMissing"));
                return;
            }
            if (
                ...
                !prompt &&
                !(context?.referenceImages.length || context?.referenceVideos.length || context?.referenceAudios.length)
            ) {
                message.warning(t("canvas.projectPage.retryPromptMissing"));
                return;
            }
            ...
            try {
                ...
                if (node.type === CanvasNodeType.Video) {
                    ...
                    const ignoreResultUrls = context ? generationReferenceUrls(context) : [];
                    const videoAssetConfig = canvasVideoAssetConfigFromAi(generationConfig, {
                        images: mediaRefHttpUrls(retryImages),
                        videos: mediaRefHttpUrls(context?.referenceVideos),
                        audios: mediaRefHttpUrls(context?.referenceAudios),
                    });
                    setNodes((prev) =>
                        prev.map((item) =>
                            item.id === node.id
                                ? {
                                      ...
                                      metadata: {
                                          ...
                                          videoTaskProvider: task.provider,
                                          videoTaskModel: task.model,
                                          references: mergeReferenceUrlLists(item.metadata?.references, task.submittedReferenceUrls, ignoreResultUrls),
                                      },
                                  }
                                : item,
                        ),
                    ...
                    const video = await storeGeneratedVideo(
                        await waitForVideoGenerationTask(
                            ...
                            videoWaitOptions(controller.signal, (progress) => {
                                setNodes((prev) =>
                                    prev.map((item) =>
                                        item.id === node.id
                                            ? { ...item, metadata: { ...item.metadata, generationProgress: progress } }
                                            : item,
                                    ),
                                );
                            }, ignoreResultUrls),
                        ...
                        {
                            onRemirrored: applyRemirroredCanvasVideo(node.id, {
                                prompt,
                                model: generationConfig.model,
                                taskId: task.id,
                                attemptId: task.attemptId,
                ...
                const retryClientId = imageId || node.metadata?.primaryImageId || nanoid();
                const { uploaded: uploadedImage, usedServerJob } = await generateCanvasImage({
                    clientId: retryClientId,
                    config: generationConfig,
                    prompt,
                    mode: useReferenceImages ? "edit" : "generation",
                    references: retryImages,
                    signal: controller.signal,
                    onServerJobStarted: () => {
                        setNodes((prev) =>
                            prev.map((item) =>
                                item.id === node.id ? { ...item, metadata: { ...item.metadata, imageJobRecoverable: true } } : item,
                            ),
                        );
                    },
                });
                ...
                syncCanvasImageGenerationAsset({
                    ...
                    config: canvasImageAssetConfigFromAi(generationConfig, {
                        mode: useReferenceImages ? "edit" : "generation",
                        images: referenceImageHttpUrls(retryImages),
                        count: 1,
                    }),
                });
                    ...
                    prev.map((item) => {
                        ...
                        return {
                            ...
                            metadata: {
                                ...
                                imageJobRecoverable: usedServerJob,
    ...
    const handleCheckVideoStatus = useCallback(
        async (node: CanvasNodeData, options?: { silent?: boolean }): Promise<boolean> => {
            if (node.type !== CanvasNodeType.Video) return false;
            const taskId = node.metadata?.videoTaskId?.trim();
            if (!taskId) {
                if (!options?.silent) message.warning(t("canvas.projectPage.videoTaskMissing"));
                return false;
            }
            if (generationRequestsRef.current.has(node.id)) {
                if (options?.silent) return true;
                // Manual click: abort the in-flight poll and start a fresh status check so Network shows a new request.
                const existing = generationRequestsRef.current.get(node.id);
                existing?.controller.abort();
                generationRequestsRef.current.delete(node.id);
                message.info(t("canvas.projectPage.videoStatusRechecking"));
            }
            const generationConfig = {
                ...buildGenerationConfig(effectiveConfig, node, "video"),
                model: node.metadata?.videoTaskModel || node.metadata?.model || effectiveConfig.videoModel || effectiveConfig.model,
            };
            if (!isAiConfigReady(generationConfig, generationConfig.model)) {
                // Keep loading on silent resume so we can retry once AI config is ready.
                if (!options?.silent) {
                    message.warning(t("canvas.projectPage.videoStatusConfigRequired"));
                    openConfigDialog(true);
                }
                return false;
            }
            ...
            const task: VideoGenerationTask = {
                ...
                model: node.metadata?.videoTaskModel || generationConfig.model,
                submittedReferenceUrls: ignoreResultUrls,
            };
            setRunningNodeId(node.id);
            setNodes((prev) =>
                prev.map((item) =>
                    item.id === node.id
                        ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_LOADING, errorDetails: undefined } }
                        : item,
                ),
            );
                ...
                if (state.status === "pending") {
                    if (typeof state.progress === "number") {
                        setNodes((prev) =>
                            prev.map((item) =>
                                item.id === node.id
                                    ? { ...item, metadata: { ...item.metadata, generationProgress: state.progress } }
                                    : item,
                            ),
                        );
                    }
                    if (!options?.silent) message.info(t("canvas.projectPage.videoStillPending", { id: taskId }));
                    const video = await storeGeneratedVideo(
                        await waitForVideoGenerationTask(
                            generationConfig,
                            task,
                    ...
                    message.success(t("canvas.projectPage.videoStatusRecovered"));
                    return true;
                }
    ...
    useEffect(() => {
        ...
        for (const node of nodes) {
            if (!isRecoverableVideoGeneration(node)) continue;
            const taskId = node.metadata?.videoTaskId?.trim();
            if (!taskId) continue;
            const resumeKey = `${node.id}:${taskId}`;
            if (resumedVideoTasksRef.current.has(resumeKey)) continue;
            if (generationRequestsRef.current.has(node.id)) continue;
            resumedVideoTasksRef.current.add(resumeKey);
            void handleCheckVideoStatus(node, { silent: true }).then((started) => {
                if (!started) resumedVideoTasksRef.current.delete(resumeKey);
            });
        }
    }, [handleCheckVideoStatus, nodes, projectLoaded]);

    const handleResumeImageJobs = useCallback(
        async (node: CanvasNodeData) => {
            if (!isRecoverableImageGeneration(node)) return false;
            if (generationRequestsRef.current.has(node.id)) return false;
            const loadingSlots =
                node.metadata?.images?.filter((image) => image.status === NODE_STATUS_LOADING).map((image) => image.id) || [];
            if (!loadingSlots.length) {
                setNodes((prev) =>
                    prev.map((item) =>
                        item.id === node.id
                            ? {
                                  ...
                                  metadata: {
                                      ...item.metadata,
                                      status: NODE_STATUS_ERROR,
                                      imageJobRecoverable: false,
                                      errorDetails: t("canvas.generation.interrupted"),
                                  },
                              }
                            : item,
                    ),
            ...
            const controller = startGenerationRequest(node.id, node.id, node.id);
            const imageConfig = NODE_DEFAULT_SIZE[CanvasNodeType.Image];
            let hasSuccess = Boolean(node.metadata?.content);
            let hasFailure = false;
            try {
                await Promise.all(
                    loadingSlots.map(async (imageId) => {
                        try {
                            const existing = await getGenerationAssetByClientId(imageId);
                            if (!existing) {
                                hasFailure = true;
                                setNodes((prev) =>
                                    prev.map((item) =>
                            ...
                            const asset =
                                existing.status === "success"
                                    ? existing
                                    : await waitForGenerationAsset(imageId, { signal: controller.signal });
                            const url = asset.assets.find((file) => file.url)?.url;
                            if (!url) throw new Error(t("canvas.projectPage.imageJobEmpty"));
                            ...
                            setNodes((prev) =>
                                prev.map((current) => {
                                    ...
                                    const images = current.metadata?.images?.map((image) => (image.id === imageId ? item : image)) || [];
                                    if (current.metadata?.primaryImageId && current.metadata.primaryImageId !== imageId) {
                                        return { ...current, metadata: { ...current.metadata, images } };
                                    }
                                    const center = {
    ...
    useEffect(() => {
        ...
        for (const node of nodesRef.current) {
            if (!isRecoverableImageGeneration(node)) continue;
            const resumeKey = node.id;
            if (resumedImageJobsRef.current.has(resumeKey)) continue;
            if (generationRequestsRef.current.has(node.id)) continue;
            resumedImageJobsRef.current.add(resumeKey);
            void handleResumeImageJobs(node).then((started) => {
                if (!started) resumedImageJobsRef.current.delete(resumeKey);
            });
        }
    }, [handleResumeImageJobs, projectLoaded]);

    const handleNodeCheckVideoStatus = useCallback((node: CanvasNodeData) => void handleCheckVideoStatus(node), [handleCheckVideoStatus]);
                ...
                >
                    ...
                    {visibleNodes.map((node) => (
                        <CanvasNode
                            ...
                            onCheckVideoStatus={handleNodeCheckVideoStatus}
