...
export default function VideoPage() {
    ...
    const generate = async () => {
        ...
        await runPool(slots, VIDEO_BATCH_CONCURRENCY, async (slot) => {
            try {
                const task = await createVideoGenerationTask(snapshot.config, requestPrompt, snapshot.references);
                const log = buildLog({
                    id: slot.id,
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-stone-50 text-stone-900 dark:bg-stone-950 dark:text-stone-100">
            ...
            <Drawer title={t("workbench.settings")} placement="bottom" styles={{ content: { height: "82vh" } }} open={settingsOpen} onClose={() => setSettingsOpen(false)}>
