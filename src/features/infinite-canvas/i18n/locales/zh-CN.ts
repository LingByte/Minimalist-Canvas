export default {
    ...
    canvas: {
        ...
        sidePanel: {
            ...
            searchLogs: "搜索生成记录", cannotInsertLog: "这条记录还没有可插入的媒体", cannotDownloadLog: "这条记录还没有可下载的媒体", mediaLinkExpired: "媒体链接已过期，正在刷新", logDownloaded: "已开始下载", logDownloadFailed: "下载失败，请重试", logDownloadHint: "已在新标签页打开下载链接，大文件可能需要一点时间，请留意浏览器下载提示哦", logsDeleted: "已删除 {{count}} 条生成记录",
    ...
    config: {
        ...
        localStorage: {
            ...
            refresh: "刷新统计",
            readFailed: "读取本地存储失败",
            syncedHint: "生图/视频历史和「我的资产」已写入数据库，可清理本地副本释放空间。画布项目、配置和媒体文件仍会保留。",
    ...
    agent: {
        ...
        connect: { pluginTitle: "方式一：在 Codex 中使用插件", pluginText: "在 Codex app 安装 至简画布 插件后，通过插件启动画布，插件会自动启动本地 Agent 并带上连接信息。", directTitle: "方式二：在 Codex CLI 中注册", directText: "运行下面命令或点击注册按钮，把画布 MCP 工具写入 Codex 配置，之后 Codex 会话即可读写当前画布。", registerMcp: "注册到 Codex", mcpRegistered: "已注册到 Codex，新会话生效", mcpServer: "本地 MCP 服务", mcpRunning: "运行中", mcpUsage: "在终端运行 codex，画布工具即可在会话中调用；画布读写操作需要先打开一个画布。", commandCopied: "命令已复制", pluginReminder: "Codex 插件提醒", pluginReminderText: "只有安装 Codex 插件或手动添加 MCP 后，工具列表才会进入 Codex 上下文并增加 token 消耗；", removePlugin: "移除插件", removeMcp: "移除手动 MCP", copyCommand: "复制命令", title: "连接本地 Agent", description: "按使用场景选择一种连接方式。", webConnection: "网页连接", autoDiscover: "默认自动读取 Local URL 和 Connect token，失败时再手动填写。", disconnect: "断开", connect: "连接", localAddress: "本地地址", urlPlaceholder: "例如 http://127.0.0.1:17371", token: "连接 Token", tokenPlaceholder: "自动发现，或手动填入 Connect token" },
        ...
        panel: { connectionSettings: "连接设置 · {{status}}", connectionSettingsLabel: "连接设置，当前{{status}}", chat: "对话", history: "历史", skills: "技能", logs: "日志", collapse: "收起对话", collapseLabel: "收起 Agent 面板", resize: "调整右侧面板宽度", content: "Agent 内容", connectHint: "未连接本地 Agent；在终端运行 codex 也能操作画布", mcpInitializing: "MCP 初始化中，完成后即可发送", initFailed: "Codex 对话初始化失败，请新建或恢复对话", placeholder: "询问 Codex，输入 / 使用技能，@ 引用画布素材" },
