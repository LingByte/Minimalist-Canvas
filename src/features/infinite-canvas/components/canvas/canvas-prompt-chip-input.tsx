...
type Props = {
    ...
    /** Picking an unconnected resource triggers this before the chip is inserted. */
...
export function CanvasPromptChipInput({ value, references, onChange, onConnectResource, className, style, placeholder }: Props) {
...
function MentionMenu({ rect, references, activeIndex, theme, onSelect }: { rect: DOMRect | null; references: CanvasResourceReference[]; activeIndex: number; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onSelect: (reference: CanvasResourceReference) => void }) {
    ...
    const renderRow = (reference: CanvasResourceReference) => {
