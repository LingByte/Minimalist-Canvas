...
import { ChevronRight, Copy, Download, Group, Image as ImageIcon, Maximize2, Music2, Pause, Play, Puzzle, RefreshCw, Search, Star, Trash2, Video } from "lucide-react";
...
type NodeContentRendererProps = {
    ...
    groupChildCount: number;
    /** When false, media content is non-interactive for dragging; overlays may still use pointer-events:auto. */
...
function NodeContent(props: NodeContentRendererProps) {
    ...
    if (props.node.metadata?.status === "loading") {
        return <LoadingContent theme={props.theme} progress={props.node.metadata?.generationProgress} />;
    }
    if (props.node.metadata?.status === "error") {
        return <ErrorContent node={props.node} theme={props.theme} onRetry={props.onRetry} onCheckVideoStatus={props.onCheckVideoStatus} />;
    }
...
function LoadingContent({ theme, progress }: Pick<NodeContentRendererProps, "theme"> & { progress?: number }) {
    const { t } = useTranslation();
    const hasProgress = typeof progress === "number" && Number.isFinite(progress) && progress > 0;
    const percent = hasProgress ? Math.max(0, Math.min(100, Math.round(progress))) : 0;
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 px-4" style={{ color: theme.node.activeStroke }}>
            <div className="size-10 animate-spin rounded-full border-2" style={{ borderColor: theme.node.stroke, borderTopColor: theme.node.activeStroke }} />
            <span className="text-[10px] tracking-[0.2em]">{t("canvas.node.generating")}</span>
            {hasProgress ? (
                <div className="w-full max-w-[140px]">
                    <div className="mb-1.5 text-center text-xs font-medium tabular-nums">{t("canvas.node.progressPercent", { value: percent })}</div>
                    <div className="h-1.5 overflow-hidden rounded-full" style={{ background: theme.node.stroke }}>
                        <div className="h-full rounded-full transition-[width] duration-300" style={{ width: `${percent}%`, background: theme.node.activeStroke }} />
                    </div>
                </div>
            ) : null}
        </div>
...
function ErrorContent({
    ...
    onRetry,
    onCheckVideoStatus,
}: Pick<NodeContentRendererProps, "node" | "theme" | "onRetry" | "onCheckVideoStatus">) {
    const { t } = useTranslation();
    const canCheckVideoStatus = node.type === CanvasNodeType.Video && Boolean(node.metadata?.videoTaskId);
    return (
        <div className="flex max-w-[280px] flex-col items-center gap-3 px-5 text-center">
            <div className="text-xs leading-5 whitespace-pre-wrap text-red-300">{node.metadata?.errorDetails || t("canvas.node.failed")}</div>
            <div className="flex flex-wrap items-center justify-center gap-2">
                {canCheckVideoStatus ? (
                    <button
                        ...
                        onClick={(event) => {
                            event.stopPropagation();
                            onCheckVideoStatus?.(node);
                        }}
                        onMouseDown={(event) => event.stopPropagation()}
                    >
                        <Search className="size-3.5" />
                        {t("canvas.node.checkStatus")}
                    </button>
                ) : null}
                <button
                    type="button"
...
function VideoMoveControls(props: {
    ...
    const seekTo = (value: number) => {
        ...
        video.currentTime = Math.max(0, Math.min(duration || video.duration || 0, value));
    ...
    return (
        <>
            {!props.playing ? (
                <button
                    type="button"
                    className="absolute left-1/2 top-1/2 z-[1] flex size-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-white/25 bg-black/55 text-white shadow-lg backdrop-blur-sm transition hover:bg-black/70"
            ...
            <div
                className="absolute inset-x-0 bottom-0 z-[1] flex items-center gap-2 bg-gradient-to-t from-black/75 via-black/45 to-transparent px-2.5 pb-2.5 pt-8 text-white"
                style={{ pointerEvents: "auto" }}
                onMouseDown={stopMediaControlEvent}
                onPointerDown={stopMediaControlEvent}
            ...
            >
                <button
                    ...
                    aria-label={props.playing ? props.pauseLabel : props.playLabel}
                    title={props.playing ? props.pauseLabel : props.playLabel}
                    onClick={(event) => {
                        stopMediaControlEvent(event);
                        event.preventDefault();
                        props.onTogglePlay();
                ...
                >
                    {props.playing ? <Pause className="size-3.5" /> : <Play className="ml-0.5 size-3.5" />}
                </button>
                ...
                <span className="shrink-0 font-mono text-[10px] tabular-nums opacity-90">
...
function VideoNodeContent({ node, theme, contentInteractive = true }: NodeContentRendererProps) {
    ...
    // Move mode uses a custom bar (play / seek / fullscreen) so Firefox/Zen can still drag the node.
    // Interact mode keeps native controls for volume and browser-specific extras.
    return (
        <div className="relative h-full w-full">
            ...
            {!contentInteractive ? (
                <VideoMoveControls
                    videoRef={videoRef}
                    playing={playing}
                    playLabel={t("canvas.node.play")}
                    pauseLabel={t("canvas.node.pause")}
                    fullscreenLabel={t("canvas.node.fullscreen")}
...
function AudioNodeContent({ node, theme, contentInteractive = true }: NodeContentRendererProps) {
    const { t } = useTranslation();
            ...
            {contentInteractive ? (
                ...
                >
                    ...
                    <button
                        ...
                        aria-label={playing ? t("canvas.node.pause") : t("canvas.node.play")}
                        title={playing ? t("canvas.node.pause") : t("canvas.node.play")}
                        onClick={(event) => {
                            stopMediaControlEvent(event);
                            event.preventDefault();
                            togglePlayback();
                    ...
                    <input
                        ...
                        value={duration > 0 ? Math.min(currentTime, duration) : 0}
                        disabled={duration <= 0}
                        className="h-1.5 min-w-0 flex-1 cursor-pointer accent-current disabled:cursor-default"
                        aria-label={t("canvas.node.play")}
                        onChange={(event) => {
                            const next = Number(event.currentTarget.value);
                            setCurrentTime(next);
                            seekTo(next);
...
function ConnectionHandleDot({ side, visible, onMouseDown }: { side: "left" | "right"; visible: boolean; onMouseDown: (event: React.MouseEvent) => void }) {
    ...
    return (
        ...
        >
            <div className="size-5 rounded-full border-[2.5px] transition-all hover:scale-110" style={{ background: theme.node.panel, borderColor: theme.node.muted }} />
        </div>
    );
}
