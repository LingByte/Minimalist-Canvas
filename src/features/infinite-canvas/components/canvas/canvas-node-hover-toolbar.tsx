...
import { Download, Ellipsis, FolderPlus, Image as ImageIcon, Info, MessageSquare, Minus, Music2, Plus, RefreshCw, Search, Settings2, Trash2, Upload, Video } from "lucide-react";
...
export function CanvasNodeHoverToolbar({
    ...
    const canCheckVideoStatus = isVideo && Boolean(node.metadata?.videoTaskId);
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...(canCheckVideoStatus ? [{ id: "checkStatus", title: t("canvas.nodeToolbar.checkStatusTitle"), label: t("canvas.node.checkStatus"), icon: <Search className="size-4" />, onClick: () => onCheckVideoStatus?.(node) }] : []),
    ...
    return (
        <>
            <div
                className="absolute z-[70] flex h-12 -translate-x-1/2 -translate-y-full items-center overflow-visible rounded-[18px] border border-black/10 bg-white text-[15px] text-stone-800 shadow-[0_8px_28px_rgba(15,23,42,.12)] dark:border-white/10 dark:bg-stone-900 dark:text-stone-100 dark:shadow-[0_8px_28px_rgba(0,0,0,.4)]"
                style={{ left, top }}
                ...
                <div className="h-[56vh] min-h-[360px] select-text text-sm" data-canvas-shortcuts-ignore>
                    {view === "info" ? (
                        <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
                            ...
                            {node.metadata?.videoTaskId ? <InfoRow label={t("canvas.projectPage.videoTaskLabel")} value={node.metadata.videoTaskId} /> : null}
    ...
    return (
        <Tooltip title={title} placement="top" mouseEnterDelay={0.2} color="#ffffff" styles={{ root: { color: "#242529", boxShadow: "0 8px 24px rgba(15,23,42,.16)", fontSize: 13, fontWeight: 500 } }}>
            <button type="button" className={`group relative flex h-12 items-center whitespace-nowrap px-1.5 ${danger ? "text-[#ef4444]" : ""}`} onClick={onClick} aria-label={title}>
                <span className={`flex h-9 items-center ${hasText ? "gap-2 px-2.5" : "justify-center px-2"} rounded-lg transition group-hover:bg-stone-100 dark:group-hover:bg-stone-700 ${active ? "bg-stone-200 dark:bg-stone-600" : ""}`}>
                    {icon}
