...
import { getGenerationResourceNodes } from "@canvas/lib/canvas/canvas-resource-references";
...
export function buildNodeGenerationContext(
    ...
    prompt: string,
    options?: { appendUpstreamText?: boolean },
    ...
    const appendUpstreamText = options?.appendUpstreamText !== false;
...
function buildComposerGenerationContext(inputs: NodeGenerationInput[], prompt: string): NodeGenerationContext {
    ...
    // No @[node:…] mentions → treat every connected resource as a reference
    // (same as non-composer generation). Empty refs here made image→video from
    // Config silently fall back to text-to-video.
    if (!hasToken) {
        const referenceImages = inputs.map((input) => input.image).filter((image): image is ReferenceImage => Boolean(image));
        const referenceVideos = inputs.map((input) => input.video).filter((video): video is ReferenceVideo => Boolean(video));
        const referenceAudios = inputs.map((input) => input.audio).filter((audio): audio is ReferenceAudio => Boolean(audio));
