...
import {
    CONTRACT_IMAGE_RESOLUTIONS,
    contractImageAspectRatios,
...
export function ImageSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5", maxCount = 15, quickCount = 10, compact = false }: ImageSettingsPanelProps) {
    ...
    const contractMode = isContractImageModel(modelName);

    if (contractMode) {
        return (
            <ContractImageSettingsPanel
                config={config}
                modelName={modelName}
...
function ContractImageSettingsPanel({
    config,
    modelName,
    onConfigChange,
    theme,
    ...
    config: AiConfig;
    modelName: string;
    onConfigChange: ImageSettingsPanelProps["onConfigChange"];
    theme: CanvasTheme;
    showTitle: boolean;
    className: string;
    compact?: boolean;
    ...
    const aspectRatio = normalizeContractAspectRatio(config.size, modelName);
    const quality = normalizeContractQuality(config.quality);
    const ratios = contractImageAspectRatios(modelName);
            ...
            >
                ...
                <div className="space-y-2.5">
                    ...
                    <div className="grid grid-cols-3 gap-2.5">
                        {CONTRACT_IMAGE_RESOLUTIONS.map((item) => (
                            <OptionPill key={item} selected={resolution === item} theme={theme} onClick={() => onConfigChange("imageResolution", item)}>
                                {item}
                            </OptionPill>
                        ))}
                    </div>
                ...
                {showQuality ? (
                    <div className="space-y-2.5">
                        ...
                        <div className="grid grid-cols-2 gap-2.5">
                            {gptImage2QualityOptions.map((item) => (
                                <OptionPill key={item.value} selected={quality === item.value} theme={theme} onClick={() => onConfigChange("quality", item.value)}>
                                    {t(`settingsPanels.common.${item.labelKey}`)}
                                </OptionPill>
                            ))}
                ...
                <div className="space-y-2.5">
                    <SettingTitle color={theme.node.muted}>{t("settingsPanels.image.count")}</SettingTitle>
                    <div className="grid grid-cols-4 gap-2.5">
                        <OptionPill selected theme={theme} onClick={() => onConfigChange("count", "1")}>
                            {t("settingsPanels.image.images", { count: 1 })}
                        </OptionPill>
                    </div>
                </div>
            </div>
...
export function imageQualityLabel(value: string, modelName?: string) {
    if (modelName && isContractImageModel(modelName) && !isGptImage2ContractModel(modelName)) {
        return normalizeContractResolution(value);
    }
...
export function contractImageSummaryLabel(config: AiConfig) {
    const modelName = config.model || config.imageModel || "";
    const resolution = normalizeContractResolution(config.imageResolution);
    const ratio = normalizeContractAspectRatio(config.size, modelName);
    if (isGptImage2ContractModel(modelName)) {
        return `${resolution} · ${normalizeContractQuality(config.quality)} · ${ratio}`;
    }
