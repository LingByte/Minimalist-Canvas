...
const secondOptions = [4, 6, 10, 12, 15, 20, 30];
...
/** Discrete presets kept for agent tools; UI uses a continuous 4–30 slider. */
export const videoSecondOptions = Array.from({ length: VIDEO_SECONDS_MAX - VIDEO_SECONDS_MIN + 1 }, (_, index) => String(VIDEO_SECONDS_MIN + index));
                    ...
                    <div className="space-y-2.5">
                        <div className="flex items-center justify-between gap-3 text-sm">
                            <span style={{ color: theme.node.muted }}>
                                {VIDEO_SECONDS_MIN}s – {VIDEO_SECONDS_MAX}s
                            </span>
                        ...
                        <input
                            ...
                            step={1}
                            value={seconds}
                            className="h-2 w-full cursor-pointer appearance-none rounded-full outline-none [&::-webkit-slider-thumb]:size-4 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border-0 [&::-moz-range-thumb]:size-4 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-0"
                            style={{
                                background: `linear-gradient(to right, ${theme.node.text} 0%, ${theme.node.text} ${((Number(seconds) - VIDEO_SECONDS_MIN) / (VIDEO_SECONDS_MAX - VIDEO_SECONDS_MIN)) * 100}%, ${theme.node.stroke} ${((Number(seconds) - VIDEO_SECONDS_MIN) / (VIDEO_SECONDS_MAX - VIDEO_SECONDS_MIN)) * 100}%, ${theme.node.stroke} 100%)`,
                                color: theme.node.text,
                                accentColor: theme.node.text,
                            }}
                            onMouseDown={(event) => event.stopPropagation()}
...
export function videoSecondsLabel(value: string) {
    ...
    return `${normalizeVideoSecondsInput(value)}s`;
}

export function normalizeVideoSecondsInput(value: string | undefined) {
