...
export function ConfigLocalStorage({ active }: { active: boolean }) {
    ...
    const confirmClearSynced = () => {
        modal.confirm({
            title: t("config.localStorage.clearSyncedTitle"),
            content: t("config.localStorage.clearSyncedDescription"),
            okText: t("config.localStorage.clearSynced"),
            okButtonProps: { danger: true },
            cancelText: t("common.cancel"),
            onOk: async () => {
                setClearing(true);
                try {
                    const result = await clearSyncedLocalData();
                    message.success(
                        t("config.localStorage.clearSyncedDone", {
                            imageLogs: result.imageLogs,
                            videoLogs: result.videoLogs,
                            assets: result.assets,
                        }),
                    );
                    await refresh();
                } catch (reason) {
                    message.error(reason instanceof Error ? reason.message : t("config.localStorage.clearSyncedFailed"));
                } finally {
                    setClearing(false);
                }
            },
            ...
            <section className="rounded-lg border border-stone-200 p-4 dark:border-stone-800">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        <Button danger icon={<Trash2 className="size-4" />} loading={clearing} onClick={confirmClearSynced}>
                            {t("config.localStorage.clearSynced")}
                        </Button>
                    </div>
                </div>
