...
async function evaluatePluginSource(source: string): Promise<CanvasPlugin> {
    ...
    try {
        // Native dynamic import — bundlers rewrite `import(url)` into their require()
        // and then fail with "Cannot find module 'blob:...'". Evaluating import through
        // Function keeps the call outside the module graph (works under Rsbuild/webpack).
        const load = new Function("url", "return import(url)") as (url: string) => Promise<{ default?: unknown; plugin?: unknown }>;
