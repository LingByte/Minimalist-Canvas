import { PLUGIN_REGISTRY_URL, REMOTE_PLUGIN_REGISTRY_URL } from "@canvas/constant/env";
...
export async function fetchOfficialPlugins(registryUrl: string = PLUGIN_REGISTRY_URL): Promise<OfficialPluginEntry[]> {
    const primary = await loadOfficialPluginRegistry(registryUrl);
    if (primary.length || registryUrl !== PLUGIN_REGISTRY_URL) return primary;
    if (registryUrl === REMOTE_PLUGIN_REGISTRY_URL) return primary;
    return loadOfficialPluginRegistry(REMOTE_PLUGIN_REGISTRY_URL);
}

async function loadOfficialPluginRegistry(registryUrl: string): Promise<OfficialPluginEntry[]> {
    ...
    const list = Array.isArray(data?.plugins) ? data.plugins : Array.isArray(data) ? (data as RawEntry[]) : [];
    return list
