  ...
  return (
    ...
    >
      <Avatar
        size={24}
        className={cn(
          'ring-border/60 shrink-0 text-[11px] font-semibold ring-1',
    ...
    <div className='space-y-2.5'>
      ...
      <div className='grid grid-cols-2 gap-1.5'>
        ...
        <SummaryField
          ...
          className='col-span-2 bg-transparent px-0 py-0'
        />
        <SummaryField label={t('Amount change')} cell={cells.get('quota')} />
        <SummaryField
          label={t('User balance')}
          cell={cells.get('user_quota')}
        />
      </div>
    </div>
  )
  ...
  if (!rows || rows.length === 0) {
    return (
      <div className='rounded-lg border p-6'>
        <Empty
          image={<Database className='text-muted-foreground size-6' />}
          description={
            <div>
              <div className='mb-1 font-semibold'>{resolvedEmptyTitle}</div>
