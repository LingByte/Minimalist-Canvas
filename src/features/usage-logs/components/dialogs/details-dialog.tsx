...
const SECTION_ICON_TONES: Record<string, string> = {
...
function DetailSection(props: {
  ...
  iconTone?: keyof typeof SECTION_ICON_TONES | string
  label: string
  variant?: 'default' | 'danger'
  children: React.ReactNode
}) {
  const isDanger = props.variant === 'danger'
  const iconTone = isDanger ? 'destructive' : (props.iconTone ?? 'neutral')
  return (
    <div className='min-w-0 space-y-1.5'>
      ...
      >
        {props.icon && (
          <span
            className={cn(
              'flex size-5 shrink-0 items-center justify-center rounded-md [&>svg]:size-3 [&>svg]:shrink-0',
              SECTION_ICON_TONES[iconTone] ?? SECTION_ICON_TONES.neutral
            )}
          >
            {props.icon}
          </span>
...
export function DetailsDialog(props: DetailsDialogProps) {
  ...
  return (
    <Modal
      ...
      footer={null}
      width={isTieredBilling ? 1024 : 512}
      destroyOnHidden
      className='min-w-0'
      styles={{
        body: {
