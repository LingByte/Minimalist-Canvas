...
export interface CommonFilters {
  ...
  endTime?: Date
  channel?: string
  /** Admin-only: selected user id from AdminUserSearchSelect */
  userId?: number
}
...
export interface TaskLog {
  ...
  channel_id: number
  /** Charged quota for this task (unsigned; may be 0 after refund). */
  quota?: number
  /**
   * Signed wallet delta for UI: negative = charged, positive = refunded.
   * Prefer this over `quota` for Amount change display.
   */
  quota_change?: number
  /**
   * Historical wallet remaining after this task's latest billing mutation.
   * Prefer `user_quota_after` when present. null/undefined = unknown (legacy).
   */
  user_quota?: number | null
  /** Persisted snapshot of wallet remaining after billing; not live current balance. */
  user_quota_after?: number | null
  ...
  fail_reason?: string
  result_url?: string
  /** Provider result link while remirror is not finished. */
  upstream_url?: string
  group?: string
  ...
  created_at?: number
  updated_at?: number
  /** Origin model name from task properties (list API). */
  model?: string
  properties?: {
