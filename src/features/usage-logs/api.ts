...
export async function adminRefreshTaskFromUpstream(taskId: string): Promise<{
  fetched?: boolean
  fetch_error?: string
  ...
  const res = await api.post(
    `/api/task/${encodeURIComponent(taskId)}/refresh`
  )
...
export async function getUserTaskDetail(taskId: string) {
  const res = await api.get(
    `/api/task/self/${encodeURIComponent(taskId)}`
  )
  const body = res.data as {
    success?: boolean
    message?: string
    data?: import('@/features/generation-assets/api').AdminTaskDetail
  }
  if (!body?.success || !body.data) {
    throw new Error(body?.message || 'failed to load task detail')
  }
  return body.data
}
