...
import { resolveApiOrigin } from '@/features/docs/lib/resolve-api-base'
...
function CodeSamplesSection(props: {
  ...
  const baseUrl = useMemo(() => resolveApiOrigin(), [])
      ...
      <div className='flex flex-wrap items-center gap-2'>
        {endpoints.length > 1 && (
          <Segmented
            size='small'
            value={endpointType}
            onChange={(v) => setEndpointType(String(v))}
            options={endpoints.map((ep) => ({ value: ep.type, label: ep.type }))}
          />
        )}
        ...
        columns={[
          {
            ...
            cell: (p) => (
              <div className='flex items-center gap-1.5'>
                ...
                {p.required && (
                  <Tag color='error'>{t('required')}</Tag>
                )}
              </div>
            ),
          },
          {
            ...
            cellClassName: tableStyles.topCell,
            cell: (p) => (
              <Tag style={{ fontFamily: 'monospace' }}>{p.type}</Tag>
            ),
          },
          {
            id: 'range',
